...
def generate_stepdb(models: List[ModelDB], connections: List[ConnectionDB],
                    min_models_in_step: int = 3) -> List[StepDB]:
    ...
    while graph:
        ...
        if len(bottom_models) == len(graph.nodes):  
            save_step = True 
        for i in range(0, len(bottom_models), min_models_in_step):
            batch_models = bottom_models[i:i + min_models_in_step]
            ...
            if save_step or len(current_step['models']) >= min_models_in_step:
